...
describe("detectPluginConfigFile", () => {
  ...
  test("prefers oh-my-openagent-gpt over legacy configs when jsonc files exist", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "oh-my-openagent-gpt.jsonc"), "{}")
    ...
    // then
    expect(result.format).toBe("jsonc")
    expect(result.path).toBe(join(testDir, "oh-my-openagent-gpt.jsonc"))
    expect(result.legacyPath).toBe(join(testDir, "oh-my-openagent.jsonc"))

    rmSync(testDir, { recursive: true, force: true })
  })

  test("falls back to oh-my-opencode when canonical configs don't exist", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "oh-my-opencode.jsonc"), "{}")
  ...
  test("loads oh-my-openagent-gpt.json before legacy json when no jsonc exists", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "oh-my-openagent-gpt.json"), "{}")
    ...
    // then
    expect(result.format).toBe("json")
    expect(result.path).toBe(join(testDir, "oh-my-openagent-gpt.json"))
  ...
  test("returns none when no config files exist", () => {
    ...
    // then
    expect(result.format).toBe("none")
    expect(result.path).toBe(join(emptyDir, "oh-my-openagent-gpt.json"))
  ...
  test("loads oh-my-openagent-gpt when only canonical jsonc exists", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "oh-my-openagent-gpt.jsonc"), "{}")
