...
describe("migrateLegacyConfigFile", () => {
  ...
  describe("#given oh-my-opencode.jsonc exists but oh-my-openagent-gpt.jsonc does not", () => {
    describe("#when migrating the config file", () => {
      it("#then writes oh-my-openagent-gpt.jsonc and renames the legacy file to a backup", () => {
        ...
        expect(existsSync(join(testDir, "oh-my-openagent-gpt.jsonc"))).toBe(true)
        expect(existsSync(legacyPath)).toBe(false)
        expect(existsSync(backupPath)).toBe(true)
        expect(readFileSync(join(testDir, "oh-my-openagent-gpt.jsonc"), "utf-8")).toBe('{ "agents": {} }')
  ...
  describe("#given a legacy config sidecar exists", () => {
    describe("#when migrating the config file", () => {
      it("#then copies applied migration history to the canonical sidecar", () => {
        ...
        const canonicalSidecarPath = join(testDir, "oh-my-openagent-gpt.json.migrations.json")
  ...
  describe("#given oh-my-opencode.json exists but oh-my-openagent-gpt.json does not", () => {
    describe("#when migrating the config file", () => {
      it("#then copies to oh-my-openagent-gpt.json", () => {
        const legacyPath = join(testDir, "oh-my-opencode.json")
        writeFileSync(legacyPath, '{ "agents": {} }')
        ...
        expect(result).toBe(true)
        expect(existsSync(join(testDir, "oh-my-openagent-gpt.json"))).toBe(true)
      })
    })
  })

  describe("#given oh-my-openagent-gpt.jsonc already exists", () => {
    describe("#when attempting migration", () => {
      it("#then returns false and does not overwrite", () => {
        const legacyPath = join(testDir, "oh-my-opencode.jsonc")
        const canonicalPath = join(testDir, "oh-my-openagent-gpt.jsonc")
