...
describe("migrateLegacyPluginEntry", () => {
  ...
  describe("#given opencode.json contains oh-my-opencode plugin entry", () => {
    describe("#when migrating the config", () => {
      it("#then replaces oh-my-opencode with oh-my-openagent-gpt", async () => {
        ...
        expect(content).toContain("oh-my-openagent-gpt@latest")
  ...
  describe("#given opencode.json contains bare oh-my-opencode entry", () => {
    describe("#when migrating the config", () => {
      it("#then replaces with oh-my-openagent-gpt", async () => {
        const configPath = join(testDir, "opencode.json")
        ...
        expect(result).toBe(true)
        const content = readFileSync(configPath, "utf-8")
        expect(content).toContain('"oh-my-openagent-gpt"')
        expect(content).not.toContain("oh-my-opencode")
      })
    })
  })

  describe("#given opencode.json contains the package-name plugin entry", () => {
    describe("#when migrating the config", () => {
      it("#then replaces it with oh-my-openagent-gpt", async () => {
        const configPath = join(testDir, "opencode.json")
        writeFileSync(configPath, JSON.stringify({ plugin: ["oh-my-opencode-gpt@latest"] }, null, 2))
        const { migrateLegacyPluginEntry } = await importFreshMigrationModule()
        ...
        const content = readFileSync(configPath, "utf-8")
        expect(content).toContain("oh-my-openagent-gpt@latest")
        expect(content).not.toContain("oh-my-opencode-gpt")
      })
    })
  })
    ...
    describe("#when migrating the config", () => {
      it("#then keeps the original config untouched and writes the migrated content to a sibling temp file", async () => {
        ...
        try {
          ...
          expect(readFileSync(tempPath, "utf-8")).toContain("oh-my-openagent-gpt@latest")
  ...
  describe("#given opencode.json contains pinned oh-my-opencode version", () => {
    describe("#when migrating the config", () => {
      it("#then preserves the version pin", async () => {
        ...
        expect(result).toBe(true)
        const content = readFileSync(configPath, "utf-8")
        expect(content).toContain("oh-my-openagent-gpt@3.11.0")
      })
    })
  })

  describe("#given opencode.json already uses oh-my-openagent-gpt", () => {
    describe("#when checking for migration", () => {
      it("#then returns false and does not modify the file", async () => {
        const configPath = join(testDir, "opencode.json")
        const original = JSON.stringify({ plugin: ["oh-my-openagent-gpt@latest"] }, null, 2)
  ...
  describe("#given plugin entries contain both canonical and legacy values", () => {
    describe("#when migrating the config", () => {
      it("#then removes the legacy entry instead of duplicating the canonical one", async () => {
        const configPath = join(testDir, "opencode.json")
        writeFileSync(configPath, JSON.stringify({ plugin: ["oh-my-openagent-gpt", "oh-my-opencode"] }, null, 2))
        const { migrateLegacyPluginEntry } = await importFreshMigrationModule()
        ...
        expect(saved.plugin).toEqual(["oh-my-openagent-gpt"])
      })
    })
  })
