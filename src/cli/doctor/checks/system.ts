...
import { isLegacyPluginEntryName, PUBLISHED_PACKAGE_NAME, PLUGIN_NAME } from "../../../shared/plugin-identity"
...
export async function checkSystem(deps: SystemCheckDeps = defaultDeps): Promise<CheckResult> {
  ...
  if (pluginInfo.entry && !pluginInfo.isLocalDev) {
    if (isLegacyPluginEntryName(pluginInfo.entry)) {
      const suffix = pluginInfo.entry.includes("@") ? pluginInfo.entry.slice(pluginInfo.entry.indexOf("@")) : ""
      const suggestedEntry = `${PLUGIN_NAME}${suffix}`
      issues.push({
        title: "Using legacy package name",
        description: `Your opencode.json references a legacy plugin name which has been renamed to "${PLUGIN_NAME}". The old names may stop working in a future release.`,
