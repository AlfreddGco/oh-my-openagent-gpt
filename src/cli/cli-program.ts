...
import { CONFIG_BASENAME, PUBLISHED_PACKAGE_NAME } from "../shared"
...
program
  .command("install")
  .description(`Install and configure ${PUBLISHED_PACKAGE_NAME} with interactive setup`)
...
Examples:
  ...
  $ bunx ${PUBLISHED_PACKAGE_NAME} install --no-tui --claude=max20 --openai=yes --gemini=yes --copilot=no
...
program
  ...
  .addHelpText("after", `
Examples:
  $ bunx ${PUBLISHED_PACKAGE_NAME} run "Fix the bug in index.ts"
  $ bunx ${PUBLISHED_PACKAGE_NAME} run --agent Sisyphus "Implement feature X"
  $ bunx ${PUBLISHED_PACKAGE_NAME} run --port 4321 "Fix the bug"
  $ bunx ${PUBLISHED_PACKAGE_NAME} run --attach http://127.0.0.1:4321 "Fix the bug"
...
Agent resolution order:
  ...
  3) ${CONFIG_BASENAME}.json "default_run_agent"
  4) Sisyphus (fallback)
...
program
  ...
  .addHelpText("after", `
Examples:
  $ bunx ${PUBLISHED_PACKAGE_NAME} get-local-version
  $ bunx ${PUBLISHED_PACKAGE_NAME} get-local-version --json
  $ bunx ${PUBLISHED_PACKAGE_NAME} get-local-version --directory /path/to/project
...
program
  .command("doctor")
  .description(`Check ${PUBLISHED_PACKAGE_NAME} installation health and diagnose issues`)
  .option("--status", "Show compact system dashboard")
  .option("--verbose", "Show detailed diagnostic information")
  .option("--json", "Output results in JSON format")
  .addHelpText("after", `
Examples:
  $ bunx ${PUBLISHED_PACKAGE_NAME} doctor            # Show problems only
  $ bunx ${PUBLISHED_PACKAGE_NAME} doctor --status   # Compact dashboard
  $ bunx ${PUBLISHED_PACKAGE_NAME} doctor --verbose  # Deep diagnostics
  $ bunx ${PUBLISHED_PACKAGE_NAME} doctor --json     # JSON output
`)
  .action(async (options) => {
...
program
  ...
  .option("-d, --directory <path>", `Working directory to read ${CONFIG_BASENAME} config from`)
...
program
  ...
  .action(() => {
    console.log(`${PUBLISHED_PACKAGE_NAME} v${VERSION}`)
  })
