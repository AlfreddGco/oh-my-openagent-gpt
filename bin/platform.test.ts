...
describe("getPlatformPackage", () => {
  ...
  test("returns darwin-arm64 for macOS ARM64", () => {
    ...
    expect(result).toBe("oh-my-opencode-gpt-darwin-arm64");
  });
  ...
  test("returns darwin-x64 for macOS Intel", () => {
    ...
    // #then returns correct package name
    expect(result).toBe("oh-my-opencode-gpt-darwin-x64");
  });
  // #endregion
  ...
  test("returns linux-x64 for Linux x64 with glibc", () => {
    ...
    // #then returns correct package name
    expect(result).toBe("oh-my-opencode-gpt-linux-x64");
  });
  ...
  test("returns linux-arm64 for Linux ARM64 with glibc", () => {
    ...
    // #then returns correct package name
    expect(result).toBe("oh-my-opencode-gpt-linux-arm64");
  });
  // #endregion
  ...
  test("returns linux-x64-musl for Alpine x64", () => {
    ...
    expect(result).toBe("oh-my-opencode-gpt-linux-x64-musl");
  });
  ...
  test("returns linux-arm64-musl for Alpine ARM64", () => {
    ...
    // #then returns correct package name with musl suffix
    expect(result).toBe("oh-my-opencode-gpt-linux-arm64-musl");
  });
  // #endregion
  ...
  test("returns windows-x64 for Windows", () => {
    ...
    expect(result).toBe("oh-my-opencode-gpt-windows-x64");
  });
  // #endregion
...
describe("getBinaryPath", () => {
  test("returns path without .exe for Unix platforms", () => {
    ...
    const pkg = "oh-my-opencode-gpt-darwin-arm64";
    ...
    expect(result).toBe("oh-my-opencode-gpt-darwin-arm64/bin/oh-my-opencode-gpt");
  });

  test("returns path with .exe for Windows", () => {
    // #given Windows platform package
    const pkg = "oh-my-opencode-gpt-windows-x64";
    ...
    expect(result).toBe("oh-my-opencode-gpt-windows-x64/bin/oh-my-opencode-gpt.exe");
  });

  test("returns path without .exe for Linux", () => {
    // #given Linux platform package
    const pkg = "oh-my-opencode-gpt-linux-x64";
    ...
    // #then returns path without extension
    expect(result).toBe("oh-my-opencode-gpt-linux-x64/bin/oh-my-opencode-gpt");
  });
});
...
describe("getPlatformPackageCandidates", () => {
  test("returns x64 and baseline candidates for Linux glibc", () => {
    ...
    expect(result).toEqual([
      "oh-my-opencode-gpt-linux-x64",
      "oh-my-opencode-gpt-linux-x64-baseline",
    ]);
  });
  ...
  test("returns x64 musl and baseline candidates for Linux musl", () => {
    ...
    expect(result).toEqual([
      "oh-my-opencode-gpt-linux-x64-musl",
      "oh-my-opencode-gpt-linux-x64-musl-baseline",
    ]);
  });
  ...
  test("returns baseline first when preferBaseline is true", () => {
    ...
    expect(result).toEqual([
      "oh-my-opencode-gpt-windows-x64-baseline",
      "oh-my-opencode-gpt-windows-x64",
    ]);
  });



  test("supports custom package family via packageBaseName override", () => {
    // #given Linux x64 with glibc and renamed package base
    const input = { platform: "linux", arch: "x64", libcFamily: "glibc", packageBaseName: "custom-plugin" };

    // #when getting package candidates
    const result = getPlatformPackageCandidates(input);

    // #then returns custom package family candidates
    expect(result).toEqual([
      "custom-plugin-linux-x64",
      "custom-plugin-linux-x64-baseline",
    ]);
  });
  test("returns only one candidate for ARM64", () => {
    ...
    expect(result).toEqual(["oh-my-opencode-gpt-linux-arm64"]);
  });
});
