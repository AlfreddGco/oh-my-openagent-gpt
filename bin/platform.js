 ...
 * @returns {string} Package name like "oh-my-opencode-gpt-darwin-arm64"
 * @throws {Error} If libc cannot be detected on Linux
 */
export function getPlatformPackage({ platform, arch, libcFamily, packageBaseName = "oh-my-opencode-gpt" }) {
  let suffix = "";
  if (platform === "linux") {
...
export function getPlatformPackageCandidates({ platform, arch, libcFamily, preferBaseline = false, packageBaseName = "oh-my-opencode-gpt" }) {
...
/** @param {{ platform: string, arch: string, libcFamily?: string | null, packageBaseName?: string }} options */
function getBaselinePlatformPackage({ platform, arch, libcFamily, packageBaseName = "oh-my-opencode-gpt" }) {
  if (arch !== "x64") {
    return null;
  }
 ...
 * @returns {string} Relative path like "oh-my-opencode-gpt-darwin-arm64/bin/oh-my-opencode-gpt"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/oh-my-opencode-gpt${ext}`;
}
