...
console.log("=== Publishing oh-my-opencode-gpt (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `oh-my-opencode-gpt-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/AlfreddGco/oh-my-openagent-gpt/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/oh-my-openagent-gpt.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
